...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  // Malloc and initialize a board_t representing the board
  board_t * b = malloc(sizeof(*b));
  b->width = w;
  b->height = h;
  b->totalMines = numMines;
  // Allocate space for 2D array "board"
  b->board = malloc(b->height * sizeof(*b->board));
  for (int ny = 0; ny < b->height; ny++) {
    b->board[ny] = malloc(b->width * sizeof(*b->board[ny]));
  }
  // Fill in all squares on the board with UNKNOWN
  for (int ny = 0; ny < b->height; ny++) {
    for (int nx = 0; nx < b->width; nx++) {
      b->board[ny][nx] = UNKNOWN;
    }
  }
...
int countMines(board_t * b, int x, int y) {
  //WRITE ME!
  int top = max(0, y - 1);                 // top limit
  int bottom = min(b->height - 1, y + 1);  // bottom limit
  int left = max(0, x - 1);                // left limit
  int right = min(b->width - 1, x + 1);    // right limit
